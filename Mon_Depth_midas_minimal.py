...
# %% [markdown]
# ### Hyperparameters
TRAIN_FIRST_N = 10  # Set to 0 to use all data, or set to a positive integer to limit the number of samples
NUM_EPOCHS = 1
...
class MiDaSDepth(nn.Module):
    def __init__(self):
        ...
        # No modification to the first conv layer as Canny input is removed.
        
    # def forward(self, x):
    #     # x is a tensor of shape (B, 3, H, W) in [0,1] or normalized
    #     # MiDaS dpt_transform expects a PIL image or numpy array in HWC, uint8 or float32 [0,1]
    #     # We'll convert each tensor in the batch to a numpy array, apply the transform, and stack

    #     # input_list = []
    #     # for i in range(x.shape[0]):
    #     #     img = x[i]
    #     #     # If normalized, unnormalize (assume ImageNet stats)
    #     #     if img.max() > 1.0 or img.min() < 0.0:
    #     #         # Already normalized, unnormalize
    #     #         mean = torch.tensor([0.485, 0.456, 0.406], device=img.device).view(3,1,1)
    #     #         std = torch.tensor([0.229, 0.224, 0.225], device=img.device).view(3,1,1)
    #     #         img = img * std + mean
    #     #     img = img.clamp(0, 1)
    #     #     img_np = img.cpu().numpy().transpose(1,2,0)  # (H, W, C)
    #     #     img_np = (img_np * 255).astype(np.uint8)  # Convert to uint8 for PIL
    #     #     pil_img = Image.fromarray(img_np)
    #     #     img_np2 = np.array(pil_img)  # Convert back to numpy array for transform
    #     #     input_tensor = self.transform(img_np2)
    #     #     input_list.append(input_tensor)
    #     # # Stack to (B, 3, H, W)
    #     # print (f"Type and shape of input_list: {type(input_list)}, {len(input_list)}")
    #     # input_tensor = torch.stack(input_list).to(x.device)
    #     prediction = self.midas_model(x) # type: ignore
    ...
    def forward(self, x: torch.Tensor):
        ...
        # x_normalized = self.norm(x_resized)
        ...
        depth = depth_resized
        depth = depth - depth.min()  # Ensure depth starts from 0
        depth = depth / (depth.max() + 1e-6)  # Normalize to [0, 1]
        depth = 1 - depth  # Invert depth: closer objects have higher values
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, device):
    ...
    for epoch in range(num_epochs):
        ...
        if True or val_loss < best_val_loss: # Original logic kept for saving "best" model (currently saves every epoch due to True)
    ...
    with torch.no_grad():
        for inputs, filenames in tqdm(test_loader, desc="Generating Test Predictions"): # Unpack directly
            ...
            for i in range(outputs.size(0)):
                ...
                pred_path = os.path.join(predictions_dir, f"{base_name}_depth.npy")
