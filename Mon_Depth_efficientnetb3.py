...
class EfficientNetB3Depth(nn.Module):
    def __init__(self, use_canny_input=False):
        super(EfficientNetB3Depth, self).__init__()
        self.use_canny_input = use_canny_input

        # Load the pre-trained EfficientNet-B3
        weights = EfficientNet_B3_Weights.IMAGENET1K_V1
        base_model = efficientnet_b3(weights=weights)

        # Modify first conv if using Canny
        if self.use_canny_input:
            ...
            # Initialize the new channel with the mean of RGB weights
            new_conv.weight.data[:, 3, :, :] = original_conv.weight.data.mean(dim=1)
            base_model.features[0][0] = new_conv

        self.encoder = base_model.features  # Features from EfficientNet-B3

        # Build a simple decoder:
        # The encoder output has 1536 channels. We reduce channels and then upsample.
    ...
    def forward(self, x):
        # Encoder
        enc_out = self.encoder(x)  # Output shape: (B, 1536, H/32, W/32)
...
def main():
    ...
    model = EfficientNetB3Depth(use_canny_input=USE_CANNY)
