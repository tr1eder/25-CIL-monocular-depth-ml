...
CANNY_REG_WEIGHT = 3        # Weight for the Canny regularization term
NUM_WORKERS = 1
...
def gradient_loss(pred_depth, canny_edge_map, device):
    ...
    # depth_grad_x = nn.functional.conv2d(pred_depth, sobel_x_kernel, padding=1)
    # depth_grad_y = nn.functional.conv2d(pred_depth, sobel_y_kernel, padding=1)
    # canny_edge_map_resized = nn.functional.interpolate(canny_edge_map, size=pred_depth.shape[-2:], mode='nearest')
    weights = (1-canny_edge_map)
    loss = torch.mean(torch.abs(depth_grad_x) * weights) + torch.mean(torch.abs(depth_grad_y) * weights)
    good_loss = torch.mean(torch.abs(depth_grad_x * canny_edge_map)) + torch.mean(torch.abs(depth_grad_y * canny_edge_map))
    loss = loss - .3 * good_loss
    return loss
...
class CombinedLoss(nn.Module):
    def __init__(self, canny_reg_weight, use_canny_regularizer=False):
        ...
        self.use_canny_regularizer = use_canny_regularizer
    def forward(self, predictions, targets, canny_edges=None, device='cpu'):
        ...
        if self.use_canny_regularizer and canny_edges is not None and self.canny_reg_weight > 0:
            ...
            edge_loss = gradient_loss(predictions, canny_edges_on_device, device)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, device, use_canny_regularizer_flag):
    ...
    for epoch in range(num_epochs):
        ...
        for data_items in tqdm(train_loader, desc="Training"):
            ...
            loss, recon_loss, edge_loss = criterion(outputs, targets, canny_edges, device)
            loss.backward()
            optimizer.step()
